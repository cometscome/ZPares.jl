! A and B are complex Hermitian and B is positive definite
program main
  use zpares
  implicit none  
#ifdef MPI
  ...
  complex(kind(0d0)), allocatable :: A(:,:), B(:,:), X(:,:)
  type(zpares_prm) :: prm
  ...
  emin = 3.99d0
  emax = 4.01d0
  call zpares_zdnshegv(prm, 'L', mat_size, A, mat_size, B, mat_size, emin, emax, num_ev, eigval, X, res, info)  
